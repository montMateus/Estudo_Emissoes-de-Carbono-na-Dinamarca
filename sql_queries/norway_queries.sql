SELECT * FROM norway.year_2021_hourly
...
-- TOTAL EMISSION SUMMARY GROUPED BY
...
SELECT 
	year_,
	SUM(total_carbon_emission) AS total_carbon_emission
FROM total_summary
GROUP BY year_
...
WITH no_2024_summary AS (
    SELECT 
        ...
        EXTRACT(HOUR FROM datetime_utc) AS hour_grouped,
        ROUND(SUM(carbon_intensity_direct)::NUMERIC, 2) AS total_direct_carbon_emission,
        COALESCE(LEAD(ROUND(SUM(carbon_intensity_direct)::NUMERIC, 2)) OVER 
		(PARTITION BY EXTRACT(MONTH FROM datetime_utc) ORDER BY EXTRACT(HOUR FROM datetime_utc)), 0) AS next_direct_carbon_emission,
        ROUND(SUM(carbon_intensity_lca)::NUMERIC, 2) AS total_lca_carbon_emission,
        COALESCE(LEAD(ROUND(SUM(carbon_intensity_lca)::NUMERIC, 2)) OVER 
		(PARTITION BY EXTRACT(MONTH FROM datetime_utc) ORDER BY EXTRACT(HOUR FROM datetime_utc)), 0) AS next_lca_carbon_emission,
        ROUND((SUM(renewable_percentage)/COUNT(renewable_percentage))::NUMERIC, 2) AS avg_renewable_percentage,
        ROUND((SUM(low_carbon_percentage)/COUNT(low_carbon_percentage))::NUMERIC, 2) AS avg_low_carbon_percentage
